...
- **curl \| sh installer:** Can detect/install Bun or Node, then `bun install -g zmail` or `npm i -g zmail`. Complements npm; same runtime choice as above.
...
- **Distribute via npm** so that `npm i -g zmail` (or `bun install -g zmail` if we stay on Bun) is the canonical install. This matches OpenClaw and Claude Code and fits the target user.
